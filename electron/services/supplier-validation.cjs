...
function supplierData(data, current) {
  v.object(data, fields);
  if (current && !Object.keys(data).length) v.invalid('Provide at least one supplier field.');
  const result = {};
...
module.exports = { supplierData, supplierFilters };
