...
export default function SalesPage() {
  ...
  return <section className="mx-auto min-w-0 max-w-screen-2xl" aria-labelledby="page-title">
    ...
    <Paper variant="outlined" sx={{ mt: 3, overflow: 'hidden' }}><div className="flex flex-wrap items-start gap-3 p-5">
      ...
      <TextField select size="small" label="Payment status" name="sale-status" value={filters.status} onChange={(e) => filter('status',e.target.value)} slotProps={{ select: { native: true } }}>{['all','paid','partial','unpaid','credit'].map((status) => <option key={status} value={status}>{status === 'all' ? 'All payments' : status}</option>)}</TextField>
