...
export default function SuppliersPage() {
  ...
  const list = useSuppliers(search, status, page);
  ...
  async function deactivate() {
    ...
    try {
      await catalogRequest(() => window.api.suppliers.deactivate(confirmation.id));
      setConfirmation(null); saved('Supplier deactivated. Existing records are preserved.');
  ...
  return (
    <section aria-labelledby="page-title" className="mx-auto max-w-screen-2xl min-w-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4"><div><h1 id="page-title" className="text-3xl font-semibold tracking-tight text-slate-900">Suppliers</h1><p className="mt-2 text-sm text-slate-500">Manage supplier contacts and keep your directory up to date.</p></div>
        <Button variant="contained" onClick={() => setEditor({ supplier: null })} disabled={!window.api?.suppliers || busy}>Add Supplier</Button></div>
      ...
      <Paper variant="outlined" sx={{ mt: 3, overflow: 'hidden', borderColor: '#e2e8f0' }}>
        <div className="flex flex-wrap gap-3 p-5"><TextField size="small" label="Search suppliers" name="supplier-search" placeholder="Name, phone or address" value={search} sx={{ flex: '1 1 240px' }} slotProps={{ htmlInput: { maxLength: 200 } }} onChange={(event) => { setSearch(event.target.value); setPage(0); }} />
          <TextField select size="small" label="Status" name="supplier-status" value={status} sx={{ minWidth: 160 }} onChange={(event) => { setStatus(event.target.value); setPage(0); }} slotProps={{ select: { native: true } }}><option value="active">Active</option><option value="inactive">Inactive</option><option value="all">All statuses</option></TextField>
          <Button onClick={() => { setSearch(''); setStatus('active'); setPage(0); }}>Reset filters</Button></div>
        {busy && <LinearProgress aria-label="Loading supplier" />}
        {list.loading ? <div className="flex min-h-64 items-center justify-center gap-3" role="status"><CircularProgress size={24} />Loading suppliers…</div>
          : list.error ? <Alert severity="error" sx={{ m: 2 }} action={<Button color="inherit" onClick={list.refresh}>Retry</Button>}>{list.error}</Alert>
            : !list.rows.length ? <div role="status" className="flex min-h-64 flex-col items-center justify-center p-6 text-center"><div className="mb-4 rounded-2xl bg-teal-50 p-4 text-teal-700"><AppIcon name="truck" size={30} /></div><h2 className="text-lg font-semibold">No suppliers found</h2><p className="mt-2 text-sm text-slate-500">Add a supplier or try different search and status filters.</p></div>
              : <TableContainer tabIndex={0} aria-label="Supplier table, scroll for more columns" sx={{ maxHeight: '60vh' }}><Table size="small" stickyHeader aria-label="Suppliers" sx={{ minWidth: 750, '& thead th': { bgcolor: '#f8fafc', fontWeight: 600 }, '& td': { py: 1.5 } }}>
      ...
      {editor && <SupplierDialog supplier={editor.supplier} onClose={() => setEditor(null)} onSaved={saved} />}
      <Dialog open={Boolean(confirmation)} maxWidth="xs" fullWidth onClose={() => { if (!busy) setConfirmation(null); }} aria-labelledby="supplier-confirm-title"><DialogTitle id="supplier-confirm-title">Deactivate supplier?</DialogTitle><DialogContent><DialogContentText>Deactivate “{confirmation?.name}”? It will be hidden from the active directory. Existing records will be preserved.</DialogContentText>{confirmationError && <Alert severity="error" sx={{ mt: 2 }}>{confirmationError}</Alert>}</DialogContent><DialogActions><Button disabled={busy} onClick={() => setConfirmation(null)}>Cancel</Button><Button color="warning" variant="contained" disabled={busy} onClick={deactivate}>{busy ? 'Deactivating…' : 'Deactivate Supplier'}</Button></DialogActions></Dialog>
