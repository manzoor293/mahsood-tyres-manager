...
export default function SaleDetails({ sale, onClose }) {
  return <Dialog open fullWidth maxWidth="md" onClose={onClose} aria-labelledby="sale-details-title"><DialogTitle id="sale-details-title">Sale {sale.invoice_number}</DialogTitle><DialogContent dividers>
    ...
    <dl className="my-5 ml-auto grid max-w-sm grid-cols-2 gap-3">{[['Subtotal',sale.subtotal],['Discount',sale.discount],['Total',sale.total],['Returned value',sale.returned_value],['Effective total',sale.effective_total],['Credit / refund due',sale.credit_due],['Paid',sale.paid_amount],['Balance',sale.balance]].map(([label,value]) => <div className="contents" key={label}><dt>{label}</dt><dd className="text-right font-semibold">{formatPrice(value)}</dd></div>)}</dl>
