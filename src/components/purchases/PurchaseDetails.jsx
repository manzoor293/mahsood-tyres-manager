...
export default function PurchaseDetails({ purchase: p, onClose }) {
  return <Dialog open fullWidth maxWidth="md" onClose={onClose} aria-labelledby="purchase-details-title">
    <DialogTitle id="purchase-details-title">Purchase {p.invoice_number}</DialogTitle><DialogContent dividers>
      ...
      <dl className="ml-auto my-5 grid max-w-sm grid-cols-2 gap-3">{[['Subtotal', p.subtotal], ['Discount', p.discount], ['Total', p.total], ['Returned value', p.returned_value], ['Effective total', p.effective_total], ['Supplier credit due', p.credit_due], ['Amount paid', p.paid_amount], ['Remaining balance', p.balance]].map(([label, value]) => <div className="contents" key={label}><dt>{label}</dt><dd className="text-right font-semibold">{formatPrice(value)}</dd></div>)}</dl>
